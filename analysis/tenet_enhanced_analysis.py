#!/usr/bin/env python3
"""
Enhanced Tenet Analysis with Detailed Keyword Tracking
Processes all Tenet episodes with granular keyword data for research analysis
"""
...
OUTPUT_DIR = "tenet_enhanced_results"
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('tenet_enhanced_analysis.log'),
...
def get_all_tenet_files():
    """Get all Tenet transcript files from GCS."""
    logger.info("📁 Discovering all Tenet files...")
    blobs = bucket.list_blobs(prefix="enriched_transcripts/")
    tenet_files = [blob.name for blob in blobs if blob.name.endswith('.json') and 'Tenet_' in blob.name]
    logger.info(f"Found {len(tenet_files)} Tenet episodes")
    return tenet_files
...
def analyze_single_episode_enhanced(file_path):
    """Enhanced analysis of a single Tenet episode with detailed keyword tracking."""
    try:
        # Load data
        ...
        result = {
            "metadata": {
                "title": data.get("episode_title", "unknown"),
...
def save_enhanced_batch_results(results, batch_num):
    """Save enhanced batch results locally and to GCS."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"tenet_enhanced_batch_{batch_num:03d}_{timestamp}.json"
    ...
    # Upload to GCS
    gcs_path = f"tenet_enhanced_analysis/{filename}"
    try:
...
def main():
    ...
    logger.info("🚀 Starting enhanced Tenet analysis with detailed keyword tracking...")
    
    # Get all Tenet files
    tenet_files = get_all_tenet_files()
    if not tenet_files:
        logger.error("No Tenet files found!")
        return
    
    logger.info(f"Processing {len(tenet_files)} Tenet episodes in batches of {BATCH_SIZE}")
    ...
    for i in range(0, len(tenet_files), BATCH_SIZE):
        batch_num = (i // BATCH_SIZE) + 1
        batch_files = tenet_files[i:i + BATCH_SIZE]
        
        logger.info(f"📊 Processing enhanced batch {batch_num}/{(len(tenet_files) + BATCH_SIZE - 1) // BATCH_SIZE} ({len(batch_files)} files)")
        ...
        if batch_results:
            save_enhanced_batch_results(batch_results, batch_num)
    ...
    summary_file = f"tenet_enhanced_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with open(summary_file, 'w') as f:
        json.dump(enhanced_summary, f, indent=2)
    
    # Upload summary to GCS
    gcs_summary_path = f"tenet_enhanced_analysis/{summary_file}"
    try:
